...
export function AppleRelayScreen() {
  ...
  // Sort: services with active connections first, then alphabetical.
    ...
    <div className="p-6">
      ...
      <div className="mb-5">
        ...
        <p className="text-tf-text/40 text-xs mt-0.5">
          {runningServices.length} running · {withConnections} active · {connectionCount} connections total
        </p>
      </div>
            ...
            <div className="mt-2 space-y-0.5 font-mono text-[10px] max-h-48 overflow-y-auto">
              {services.map((s) => (
                <div key={s.name} className="flex gap-2">
                  <span className="w-24 truncate shrink-0">{s.state}</span>
                  <span>{s.display_name}</span>
                  {s.pid && <span className="text-tf-text/20">PID {s.pid}</span>}
                </div>
              ))}
            </div>
          </details>
